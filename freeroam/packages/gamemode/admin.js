...
admin.addWhitelist = function(player, social) {
    try {
        if (!user.isAdmin(player))
            return;

        if(!enums.whitelistState)
        {
            player.notify('~r~Whitelist отключен в данный момент (enums.whitelistState).');
            return;
        }
        
        social = methods.removeSpecialChars(methods.removeQuotes2(methods.removeQuotes(social)));
        let sql = `SELECT * FROM white_list WHERE social = '${social}' LIMIT 1`;

        mysql.executeQuery(sql, function (err, rows, fields) {
            if(rows.length != 0)
            {
                player.notify('~r~Этот логин Social Club уже присутствует в Whitelist');
                return;
            }
            else
            {
                mysql.executeQuery(`INSERT INTO white_list (social) VALUES ('${social}')`);
                chat.sendToAll(`Администратор ${user.getRpName(player)}`, `Логин SC - ${social} был занесён в белый список!`, chat.clRed);
            }
        });
    }
    catch (e) {
...
admin.removeWhitelist = function(player, social) {
    try {
        if (!user.isAdmin(player))
            return;
        ...
        mysql.executeQuery(sql, function (err, rows, fields) {
            if(rows.length == 0)
            {
                player.notify('~r~Этот логин Social Club на найден в Whitelist');
                return;
            }
            else
            {
                mysql.executeQuery(`DELETE FROM white_list WHERE social = '${social}'`);
                chat.sendToAll(`Администратор ${user.getRpName(player)}`, `Логин SC - ${social} был удален из белого списка!`, chat.clRed);
            }
        });
    }
    catch (e) {
