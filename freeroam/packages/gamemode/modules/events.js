...
mp.events.add('server:clientDebug', (player, message) => {
    try {
        ...
        methods.saveFile('debug', `[DEBUG-CLIENT][${player.socialClub}]: ${message}`);
    } catch (e) {
        console.log(e);
    }
});
...
/*
mp.events.addRemoteCounted('server:user:taskCarryById', (player, targetId) => {
    if (!user.isLogin(player))
        return;
    ...
    if (mp.players.exists(target)) {
        ...
        if (!user.isTie(target) && !user.isCuff(target)) {
            player.notify('~r~Игрок должен быть связан или в наручниках');
            return;
        }
        if (user.get(player, 'jail_time') > 0 || user.get(target, 'jail_time') > 0)
        ...
        if (user.get(player, 'stats_strength') < 96) {
            player.notify("~r~Необходимо иметь навык силы больше 50%");
            return;
        }
        if (user.get(target, 'online_time') < 170) {
            player.notify("~r~Данный игрок новичок, по отношению к нему запрещены такие действия");
        ...
        player.setVariable('carry', target.id);
        user.playAnimation(player, "missfinale_c2mcs_1", "fin_c2_mcs_1_camman", 49);
        
        user.playAnimation(target, "nm", "firemans_carry", 33);
        target.alpha = 128; // 0

        //nplayer.call("client:taskCarry", [player]);
    }
...
/*mp.events.addRemoteCounted('server:admin:whitelist_add', (player, social) => {
    methods.debug(`server-whitelist-add: ${social}`);
    admin.addWhitelist(player, social);
});

mp.events.addRemoteCounted('server:admin:whitelist_remove', (player, social) => {
    methods.debug(`server-whitelist-remove: ${social}`);
    admin.removeWhitelist(player, social);
});*/
