    ...
    mysql.executeQuery(`SELECT ${selectSql} FROM users WHERE name = ? LIMIT 1`, name, function (err, rows, fields) {
        ...
        setTimeout(function() {
            ...
            setTimeout(function () {
                ...
                player.setVariable('idStatic', user.get(player, 'id'));
    ...
    try {
        ...
        if (user.get(player, 'mask') >= 0 && user.get(player, 'mask_color') >= 1) {
            ...
            if (mask[10]) {
                ...
                methods.debug('user.updateCharacterFace (SKIN_FACE_SPECIFICATIONS): ', methods.isValidJSON(skin.SKIN_FACE_SPECIFICATIONS));
...
user.warn = function(player, count, reason, notify = true) {
    ...
    if (count < 0) {
        mysql.executeQuery(`INSERT INTO ban_list (ban_from, ban_to, count, datetime, reason) VALUES ('Server', '${user.getRpName(player)}', 'unwarn', '${methods.getTimeStamp()}', '${reason}')`);
        ...
        mysql.executeQuery(`INSERT INTO ban_list (ban_from, ban_to, count, datetime, reason) VALUES ('Server', '${user.getRpName(player)}', 'warn', '${methods.getTimeStamp()}', '${reason}')`);
