...
admin.noClip = function(enable) {
    try {
        ...
        if (noClipEnabled)
            mp.game.ui.notifications.show(`~b~Tıklayın ~s~H~b~ kapatmak için No Clip`);
...
admin.godmode = function(enable, notify = true) {
    try {
        ...
        {
            if (godmodeEnabled)
                mp.game.ui.notifications.show(`~q~Tanrı Modu etkinleştirildi`);
            else
                mp.game.ui.notifications.show(`~q~Tanrı Modu devre dışı bırakıldı`);
        }
