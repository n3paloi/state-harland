...
inventory.show = function() {
    ...
    try {
        ...
        mp.game.ui.notifications.show(`~b~Envanteri gizle ~s~${bind.getKeyName(user.getCache('s_bind_inv'))}`);
...
inventory.openInventoryByEntity = async function(entity) {
    ...
    if (entity.getType() == 2) {
        ...
        try {
            ...
            if (entity.isDead()) {
                mp.game.ui.notifications.show("~r~Ulaşım yok edildi");
                ...
                mp.game.ui.notifications.show("~g~Bagajın yakınında olmalısınız");
            } else if (entity.getVariable('useless') === true) {
                mp.game.ui.notifications.show("~r~Bu ulaşım mevcut değil");
            }
            else {
            ...
            methods.debug(e);
            mp.game.ui.notifications.show("~r~Bilinmeyen bir hata oluştu #9998");
        }
    }
        ...
        try {
            if (entity.getVariable('isDrop'))
                ...
                let pass = methods.parseInt(await UIMenu.Menu.GetUserInput("Pın kodunu girin", "", 5));
                if (pass === data.get('is_safe'))
                    inventory.getItemList(inventory.types.House, entity.getVariable('houseSafe'));
                else
                    mp.game.ui.notifications.show('~r~Yanlış pın kodunu girdiniz mi');
            }
...
inventory.takeNewItem = async function(itemId, params = "{}", count = 1) { //TODO
    try {
        ...
        if (items.getItemAmountById(itemId) + amount > amountMax) {
            ...
            mp.game.ui.notifications.show("~r~Envanter dolu");
            return;
        }
        inventory.addItem(itemId, 1, inventory.types.Player, user_id, count, 0, params, 1);
        inventory.updateAmount(user_id, inventory.types.Player);
        mp.game.ui.notifications.show(`~b~Yakaladınız \"${items.getItemNameById(itemId)}\"`);
        chat.sendMeCommand(`alıyordu \"${items.getItemNameById(itemId)}\"`);
    }
    catch (e) {
        methods.debug(e);
    }
...
inventory.takeNewWeaponItem = async function(itemId, params, text = 'Silah alındı', count = -1) { //TODO
    try {
        let user_id = user.getCache('id');
        let amount = await inventory.getInvAmount(user_id, inventory.types.Player);
        let amountMax = await inventory.getInvAmountMax(user_id, inventory.types.Player);
        ...
        if (items.isWeaponComponent(itemId)) {
            ...
            {
                mp.game.ui.notifications.show("~r~Stokta yeterli modül yok");
                return;
            }
