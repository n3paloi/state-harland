...
menuList.showPlayerDoMenu = function(playerId) {
    ...
    UIMenu.Menu.AddMenuItem("Взять на плечо", "", {eventName: "server:user:taskCarryById"});
        ...
        try {
            for (const [key, value] of Object.entries(upgradeList)) {
                ...
                if (mod >= 100) {
                    ...
                    */
                    let label = `${enums.lscNames[mod][0]} #${(val + 1)}`;
...
menuList.showAdminMenu = function() {
    ...
    if (user.isAdmin()) {
        ...
        if (mp.players.local.getVariable('enableAdmin') === true) {
            ...
            if (user.isAdmin(5)) {
                ...
                UIMenu.Menu.AddMenuItem("~r~Добавить в Whitelist", "", {doName: "whitelist_add"});
                UIMenu.Menu.AddMenuItem("~r~Удалить из Whitelist", "", {doName: "whitelist_remove"});
            }
        }
        else {
    ...
    UIMenu.Menu.OnSelect.Add(async item => {
        ...
        if (item.doName == 'whitelist_add') {
            try {
                let social = await UIMenu.Menu.GetUserInput("Логин Social Club", "", 128);
                if (social === '')
                    return;
                methods.debug(`social_add: ${social}`);
                mp.events.callRemote('server:admin:whitelist_add', social);
            }
            catch (e) {
                methods.debug(e);
            }
        }
        if (item.doName == 'whitelist_remove') {
            try {
                let social = await UIMenu.Menu.GetUserInput("Логин Social Club", "", 128);
                if (social === '')
                    return;
                methods.debug(`social_remove: ${social}`);
                mp.events.callRemote('server:admin:whitelist_remove', methods.removeQuotes(social));
            }
            catch (e) {
                methods.debug(e);
            }
...
menuList.showAdminTunningMenu = function() {
    ...
    for (let i = 0; i < 100; i++) {
        try {
            ...
            if (veh.getNumMods(i) > 0 && enums.lscNames[i][1] > 0) {
                // rage mp 1.1 (gta tuners update) bug
                /*let label = mp.game.ui.getLabelText(veh.getModSlotName(i));
                if (label == "NULL" || label == "")
                    label = `${enums.lscNames[i][0]}`;
                */
                let label = `${enums.lscNames[i][0]}`;
