    ...
    if (entity && mp.vehicles.exists(entity)) {
        ...
        if (typeof actualData !== 'undefined') {
            try {
                ...
                if(!actualData.Engine) mp.game.invoke(methods.SET_VEHICLE_UNDRIVEABLE, entity.handle, true); // Машина глушится когда сели в нее (если двигатель заглушен ок да)
    ...
    try {
        ...
        if (entity.type === "vehicle") {
            ...
            try {
                ...
                //mp.game.invoke(methods.SET_VEHICLE_UNDRIVEABLE, entity.handle, true); // Машина сама глушится при выходе из стрима
                vSync.updateValues(entity);
